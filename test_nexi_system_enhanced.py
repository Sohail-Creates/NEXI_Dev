    ...
    def compact_conversation_history(self, user_id: str, full_history: List[Dict]) -> List[Dict]:
        ...
        try:
            ...
            with self.manager_lock:
                ...
                # Update stats (ensure user_stats[user_id] exists for new users - FIX Turn 1 LLM failure)
                if user_id not in self.user_stats:
                    self.user_stats[user_id] = {}
...
def start_wake_word_detection(duration: int = 30) -> bool:
    ...
    NOTE: Duration parameter is NOT passed to endpoint - it's for local timing only.
    The endpoint starts continuous listening in background and returns immediately.
    """
    try:
        # Call endpoint WITHOUT duration parameter (endpoint doesn't accept it)
        # The endpoint starts listening and returns immediately
        response = requests.post(
        ...
        if response.status_code == 200:
            ...
            print_info(f"Listening for 'Hey Nexi' for up to {duration} seconds...")
            return True
        else:
            # Print detailed error response
            try:
                error_detail = response.json()
                print_error(f"Failed to start wake word detection: {response.status_code}")
        ...
        print_error(f"Wake word detection timeout (service not responding within 10s)")
        return False
    except requests.ConnectionError:
        print_error(f"Could not connect to audio service at {ServiceConfig.AUDIO_API}")
        return False
    except Exception as e:
...
# ============================================================================
# CONTINUOUS CONVERSATION API HELPERS - Voice-Only Interaction with Stop Word
# ============================================================================

def set_detection_mode(mode: str) -> bool:
    """
    Switch Audio Service between idle (wake word) and conversation (stop word) modes.
    
    Args:
        mode: "idle" (wake word active) or "conversation" (stop word active)
    
    Returns:
        bool: True if mode switched successfully
    """
    try:
        print_info(f"Switching detection mode to: {mode}")
        response = requests.post(
            f"{ServiceConfig.AUDIO_API}/set-detection-mode",
            json={"mode": mode},
            timeout=10
        )
        ...
        if response.status_code == 200:
            ...
            if result.get('success'):
                print_success(f"Detection mode: {result.get('mode')} (wake_word={result.get('wake_word_active')}, stop_word={result.get('stop_word_active')})")
                return True
            else:
                print_error(f"Mode switch failed: {result.get('message', 'Unknown error')}")
                return False
        else:
            print_error(f"Mode switch failed: HTTP {response.status_code}")
            try:
                error_data = response.json()
                print_error(f"  Detail: {error_data.get('detail', {}).get('error', str(error_data))}")
            except:
                pass
            return False
    
    except requests.Timeout:
        print_error("Detection mode change timeout")
        return False
    except Exception as e:
        print_error(f"Error switching detection mode: {str(e)}")
        return False


def poll_stop_word(timeout_seconds: float = 0.1) -> tuple:
    """
    Poll for stop word detection without blocking.
    
    Args:
        timeout_seconds: How long to wait for stop word (0.1s for non-blocking checks)
    
    Returns:
        tuple: (stop_word_detected: bool, event_data: dict or None)
    """
    try:
        response = requests.get(
            f"{ServiceConfig.AUDIO_API}/poll-stop-word",
            params={"timeout": timeout_seconds},
            timeout=timeout_seconds + 1  # API timeout should be slightly longer
        )
        ...
        if response.status_code == 200:
            ...
            if event_detected and event:
                keyword = event.get('keyword', 'stop_word')
                print_info(f"[STOP WORD DETECTED] Keyword: '{keyword}'")
                return True, event
            else:
                # No event, but no error - just keep listening
                return False, None
        else:
            print_warning(f"Stop word poll failed: HTTP {response.status_code}")
            return False, None
    
    except requests.Timeout:
        # Timeout is expected for non-blocking polls - just means no event yet
        return False, None
    except Exception as e:
        print_warning(f"Stop word polling error: {str(e)[:40]}")
        return False, None


def record_vad_until_silence() -> Optional[Dict]:
    """
    Record audio using VAD (Voice Activity Detection) until silence.
    
    Returns:
        dict with keys: {'success': bool, 'audio_file': str, 'duration': float, 'stopped_by': str}
        or None if failed
    """
    try:
        response = requests.post(
            f"{ServiceConfig.AUDIO_API}/record-until-silence",
            timeout=35  # VAD recording can take up to 30 seconds + buffer
        )
        ...
        if response.status_code == 200:
            ...
            if result.get('success'):
                ...
                print_success(f"Recording: {duration:.1f}s (stopped by: {stopped_by})")
                ...
                print_error(f"VAD recording failed: {error_msg}")
                return None
        else:
            print_error(f"VAD recording failed: HTTP {response.status_code}")
            try:
                error_data = response.json()
                print_error(f"  Detail: {error_data.get('detail', {}).get('error', str(error_data))}")
            except:
        ...
        print_error("VAD recording timeout (user did not speak or spoke too long)")
        return None
    except Exception as e:
        print_error(f"VAD recording error: {str(e)}")
        return None
...
def stop_wake_word_detection() -> bool:
    ...
    try:
        ...
        if response.status_code == 200:
            print_success("Wake word detection stopped")
            return True
        else:
            print_error(f"Failed to stop wake word detection: {response.status_code}")
            return False
    
    except requests.Timeout:
        print_warning("Stop request timeout (service may already be stopped)")
        return True  # Treat as success since we're trying to stop anyway
    except requests.ConnectionError:
        print_warning(f"Could not connect to audio service (may already be down)")
...
def build_personalized_user_context(user_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        taught_objects_full = get_taught_objects(user_id)
        # ROOT FIX: Limit to top 10 taught objects to prevent bloat when serialized
        # Extract ONLY essential fields (name, type) to minimize object size
        taught_objects = [{"name": obj.get('name', 'Unknown')[:50], 
                          "type": obj.get('type', 'Unknown')[:30]} 
                         for obj in taught_objects_full[-10:]]  # Only last 10, minimal fields
        
        # Extract user preferences/interests from conversation history
        user_interests = _extract_user_interests_from_history(compacted_history, taught_objects_full)
...
def conduct_single_conversation(user_id: str, verified_context: bool = False, audio_file: str = None):
    ...
    try:
        ...
        formatted_knowledge = []
        if knowledge:
            # FIXED ROOT CAUSE: Aggressive truncation of knowledge fields to prevent prompt bloat
            for item in knowledge:
                formatted_item = {
                    "object_label": item.get('object_name', 'Unknown')[:40],  # Truncate to 40 chars
                    "category": item.get('category', '')[:30] if item.get('category') else None,  # Truncate to 30 chars
                    "type": item.get('type', '')[:20] if item.get('type') else None,  # Truncate to 20 chars
                    "confidence_match": round(item.get('confidence', 0), 2),
                    "tags": [tag[:20] for tag in item.get('tags', [])[:3]]  # Only 3 tags, each max 20 chars
                }
        ...
        llm_context = context_builder.build_context(
            ...
            additional_metadata={
                # FIX ROOT CAUSE: Only send essential metadata fields (REMOVED all verbose fields)
                # Removed: context_window_original_size, is_heavy_user, context_token_budget,
                #          current_tokens_used, sliding_window_enabled, context_window_size,
                #          interest_topics, taught_categories (all verbose/redundant)
                
                # Only limit to top 5 taught objects, extract ONLY name and type (not full objects)
                "taught_objects": [{"name": obj.get('name', 'Unknown')[:50], 
                                   "type": obj.get('type', 'Unknown')[:30]} 
                                  for obj in taught_objects[-5:]],  # Max 5 recent, minimal fields
                "user_interests": user_interests,  # Keep: User's hobbies and interests
                "knowledge_search_status": knowledge_search_status,  # Keep: Search result status
                "knowledge_context_note": knowledge_context_note,  # Keep: Explicit guidance for LLM
                "last_interaction": last_interaction,  # Keep: For warm greeting logic
                "was_context_compacted": personalized_context.get('was_compacted', False),  # Keep: Important for reasoning
                "user_profile_available": True  # Keep: Flag for context availability
            }
        )
...
def menu_return_user_conversation():
    """
    Return User Conversation - SARA
    
    CONTINUOUS VOICE-ONLY CONVERSATION with automatic stop word detection.
    
    Pipeline:
    1. Wake word detection: Listen for "Hey NEXI"
    2. Speaker verification: Identify user
    3. CONVERSATION LOOP (automatic, voice-only):
       a. Poll for stop word (quick check)
       b. If stop word detected → exit loop
       c. Otherwise → record query via VAD (Voice Activity Detection)
       d. Process through full pipeline (STT, vision, knowledge, LLM, TTS)
       e. Loop back to (a)
    4. Exit: Switch back to idle mode, play goodbye
    
    NO KEYBOARD PROMPTS - entire flow is voice-only!
    """
    print_header("RETURN USER CONVERSATION - Sara")
    print_info("Full pipeline: Wake → Verify → [Loop: STT → Vision → Knowledge → LLM → TTS]")
    print_info("Continuous conversation with automatic stop word detection - no manual prompts!")
    ...
    print_step(1, 3, "Wake Word Detection")
    ...
    try:
        ...
        if response.status_code != 200:
            print_error(f"Wake word detection failed with status code: {response.status_code}")
            try:
                error_data = response.json()
                print_error(f"  Error: {error_data.get('detail', {}).get('message', str(error_data))}")
            except:
        ...
        print_success(f"Wake word detected ({detection_time:.2f}s)")
    ...
    print_step(2, 3, "Speaker Verification")
    ...
    # ========== PHASE 3: SWITCH TO CONVERSATION MODE ==========
    # Deactivate wake word, activate stop word detection
    print_step(3, 3, "Enter Conversation Mode")
    
    if not set_detection_mode("conversation"):
        print_error("Failed to switch to conversation mode")
        return
    
    print_success("Conversation mode active")
    print_info("Say your questions - I will listen continuously")
    print_info("Say 'Stop NEXI' when done to exit")
    
    # ========== PHASE 4: CONTINUOUS CONVERSATION LOOP ==========
    print_header("CONVERSATION LOOP - SARA")
    
    turn = 1
    max_turns = 10  # Safety limit to prevent infinite loops (user can always say "stop nexi")
    in_conversation = True
    first_turn = True
    
    # CRITICAL FIX: Reuse verification audio as Turn 1 (eliminate duplication)
    # Instead of recording the first query twice:
    # - First time: for speaker verification
    # - Second time: for Turn 1 conversation
    # Now: Use SAME audio for both verification AND Turn 1
    first_turn_audio = command_file  # Reuse verification recording for Turn 1
    
    while in_conversation and turn <= max_turns:
        print(f"\n--- Turn {turn} ---")
        
        # STEP 1: Check for stop word (non-blocking poll)
        print_info("Waiting for query or stop word...")
        stop_detected, stop_event = poll_stop_word(timeout_seconds=0.1)
        
        if stop_detected:
            print_success("Stop word detected, exiting conversation")
            in_conversation = False
            break
        
        # STEP 2: Record user query using VAD (Voice Activity Detection)
        # OPTIMIZATION: For first turn, reuse verification audio (no duplication)
        # For subsequent turns, record normally
        if first_turn and first_turn_audio:
            print_info("Using verification audio as first query (no duplication)")
            # Reuse the verification recording as Turn 1
            recording_result = {
                'success': True,
                'audio_file': first_turn_audio,
                'duration': AudioConfig.DEFAULT_DURATION,  # Was already recorded during verification
                'stopped_by': 'verification_audio_reuse',
                'chunks_recorded': int(AudioConfig.DEFAULT_DURATION * 16000 / 512)
            }
            first_turn = False
            audio_file = recording_result.get('audio_file')
            duration = recording_result.get('duration', 0)
            print_success(f"Recording: {duration:.1f}s (reused from speaker verification)")
        else:
            # Subsequent turns: record normally with VAD
            print_info("Recording your question...")
            
            recording_result = record_vad_until_silence()
            
            if not recording_result:
                print_error("Failed to record query")
                # Check again for stop word before continuing
                stop_detected, _ = poll_stop_word(timeout_seconds=0.1)
                if stop_detected:
                    print_info("Stop word detected during recording error")
                    in_conversation = False
                continue
        ...
        # STEP 3: Validate recording (ignore very short recordings as noise)
        if duration < 0.5:
            print_warning(f"Recording too short ({duration:.1f}s), skipping as noise")
            # Don't break loop, just continue listening
            turn += 1
            continue
        
        # STEP 4: Process query through full conversation pipeline
        print_step(1, 5, "Speech-to-Text Transcription")
        ...
        if not transcription:
            ...
            voice_player.play_voice("transcribe_failed")
            turn += 1
            turn_skip_sleep = True
            continue
        
        print_success(f"Transcription: {transcription}")
        
        # CHECKPOINT 2: Check for stop word in transcription
        # Normalize text to handle punctuation that STT adds ("Stop, Nexi." -> "stop nexi")
        normalized_text = re.sub(r'[.,!?;:]', '', transcription.lower())
        if any(phrase in normalized_text for phrase in ["stop nexi", "stop nexie", "exit", "quit"]):
            print_success("[STOP WORD] Detected in transcription - exiting")
            in_conversation = False
            break
        
        print_step(2, 5, "Get Vision Context (Mood)")
        mood = capture_mood_with_visual_feedback()
        print_result("Detected Mood", mood)
        
        print_step(3, 5, "Query Knowledge Base (TeachMe)")
        knowledge = []
        knowledge_search_status = "no_entities"
        try:
            entities = QueryProcessorService.extract_entities(transcription)
        ...
        print_step(4, 5, "Generate LLM Response & Speak")
        llm_response = "I am thinking about your question."
        llm_source = "fallback"
        
        try:
            language_detected = "en"
            if any(ord(c) > 127 for c in transcription):
                language_detected = "ur"
            ...
            formatted_knowledge = []
            if knowledge:
                # FIXED: Limit knowledge items to top 2 for prompt character budget (was unlimited, now max 2)
                # PLUS: Aggressive truncation of field values to prevent bloat
                for item in knowledge[:2]:
                    formatted_item = {
                        "object_label": item.get('object_name', 'Unknown')[:40],  # Truncate name to 40 chars
                        "category": item.get('category', '')[:30] if item.get('category') else None,  # Truncate to 30 chars
                        "type": item.get('type', '')[:20] if item.get('type') else None,  # Truncate to 20 chars
                        "confidence_match": round(item.get('confidence', 0), 2),
                        "tags": [tag[:20] for tag in item.get('tags', [])[:3]]  # Only 3 tags, each max 20 chars
            ...
            llm_context = context_builder.build_context(
                ...
                conversation_history=conversation_history,
                additional_metadata={
                    # FIXED: Removed redundant/verbose fields to reduce character count:
                    # - Removed: context_window_original_size (duplicate info)
                    # - Removed: is_heavy_user (LLM doesn't need this)
                    # - Removed: context_token_budget (verbose technical data)
                    # - Removed: current_tokens_used (verbose technical data)
                    # - Removed: sliding_window_enabled (technical flag)
                    # - Removed: taught_categories & interest_topics (covered by user_interests)
                    
                    "taught_objects": [{"name": obj.get('name'), "type": obj.get('type')} 
                                      for obj in taught_objects[-3:]],  # Only last 3 objects to save characters
                    "user_interests": user_interests,
                    "knowledge_search_status": knowledge_search_status,
                    "knowledge_context_note": knowledge_context_note,
                    "last_interaction": last_interaction,
                    "was_context_compacted": personalized_context.get('was_compacted', False),
            ...
            if response.status_code == 200:
                ...
                if response_json.get('success'):
                    ...
                    response_language = "en"
                    if any(ord(c) > 127 for c in llm_response):
                        response_language = "ur"
                    ...
                    # ENHANCEMENT: Show actual LLM error instead of silent failure
                    error_msg = response_json.get('error', response_json.get('message', 'Unknown error'))
                    error_details = response_json.get('details', '')
                    if error_details:
                        print_error(f"LLM error: {error_msg} - {error_details}")
                    else:
                        print_error(f"LLM error: {error_msg}")
                    print_warning("Using fallback response")
            else:
                print_warning(f"LLM unavailable (HTTP {response.status_code})")
        
        except requests.Timeout:
            print_error("LLM request timeout - service not responding within 160 seconds")
            print_warning("Using fallback response")
        
        except Exception as e:
            # ENHANCEMENT: Show full error for debugging
            error_str = str(e)
            if len(error_str) > 200:
                print_error(f"LLM exception: {error_str[:200]}...")
            else:
                print_error(f"LLM exception: {error_str}")
            print_warning("Using fallback response")
        
        print_step(5, 5, "Complete & Continue Listening")
        print_result("User", user_id)
        print_result("Command", f'"{transcription}"')
        print_result("Mood", mood)
        print_result("Knowledge Items", len(knowledge))
        ...
        # CRITICAL: Do NOT prompt user here!
        # Just loop back to check stop word and record next query
        # No input(), no manual prompts - fully automatic!
        
        # CHECKPOINT 3: Post-response stop word poll
        # Give user chance to say stop word while system is between responses
        stop_detected, _ = poll_stop_word(timeout_seconds=0.1)
        if stop_detected:
            print_success("[STOP WORD] Detected post-response - exiting")
            in_conversation = False
            break
        
        turn += 1
    
    # ========== PHASE 5: EXIT CONVERSATION MODE ==========
    print_header("EXITING CONVERSATION MODE")
    
    if not set_detection_mode("idle"):
        print_warning("Failed to switch back to idle mode (non-critical)")
    
    # Play goodbye message
    try:
        goodbye_text = "Goodbye! Call me anytime."
        if synthesize_and_play_tts(goodbye_text, language="en"):
            print_success("Goodbye message played")
        else:
            print_info("Goodbye message not played")
    except Exception as e:
        print_warning(f"Could not play goodbye: {str(e)[:40]}")
